...
\subsection{Stopped Branches (Gate-Safe but Leaderboard Failed or Tied)}

\begin{table}[H]
\centering
\caption{Experiment branches stopped after public feedback (June 2026).}
\label{tab:stopped}
\begin{tabular}{@{}llcl@{}}
\toprule
\textbf{Branch} & \textbf{Representative file} & \textbf{Public MAE} & \textbf{Why stopped} \\
\midrule
Season ordinal prob blend & \texttt{prob\_blend\_recycle8088\_season\_ord08} & 0.8103 & Worse than 0.8088 anchor \\
Blackout ordinal prob blend & \texttt{prob\_blend\_recycle8088\_blackout\_ord07} & 0.8135 & Much worse despite safe gate \\
Hybrid\_full ordinal 3\% & \texttt{prob\_blend\_8089\_fullord03} & 0.8109 & Gate safe; LB +0.0021 \\
Fullord recycle & \texttt{prob\_blend\_recycle8089\_fullord03\_ord02} & 0.8123 & Second recycle amplified damage \\
Region\_state AR-lag 3\% & \texttt{prob\_blend\_recycle8088\_region\_state\_w003} & 0.8121 & Best OOF (0.206); OOF$\rightarrow$test disconnect \\
OOF-cal hybrid ordinal 8\% & \texttt{prob\_blend\_8089\_oofcal\_ord08\_full} & 0.8091 & OOF improved; mean shift down \\
Scalar temporal holdout cal & \texttt{scalar\_cal\_holdout\_full} & 0.8112 & Holdout proxy improved; LB worse \\
MAE quantile decoder $q{=}0.52$ & \texttt{prob\_mae\_q052\_8089\_ord08\_full} & 0.9206 & Invalid decoder on soft cache \\
Season/history substitution & \texttt{prob\_subst\_season\_900\_802\_full} & 0.8088 (tie) & No LB gain \\
Recycle2 on 8088 anchor & \texttt{prob\_blend\_recycle2\_8088\_w0.97} & 0.8089 & Small regression \\
PatchTST standalone & \texttt{patchtst\_*} & 1.04+ & Severe overfit \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Remaining Gap and PatchTST}

Top public leaderboard teams score between 0.7628 and 0.8043; our best is 0.8088 (gap to course target 0.8056: 0.0032). PatchTST with self-supervised pretraining was implemented and tested but severely overfit (public MAE 1.04+), matching the V2 CNN failure mode. Further prob-blend micro-sweeps have plateaued. Any future gain likely requires joint region-history retraining with temporal validation, not another post-hoc cache average.
...
\textbf{Best submission file:} \texttt{output/daily\_candidates/prob\_blend\_recycle8089\_ord08.csv} (public MAE 0.8088).

\textbf{Validate format and gate (self-check):}
\begin{verbatim}
PYTHONPATH=. python scripts/gate_and_validate_submissions.py \
  --reference output/daily_candidates/prob_blend_recycle8089_ord08.csv \
  --candidate output/daily_candidates/prob_blend_recycle8089_ord08.csv
\end{verbatim}
Expected: \texttt{validate\_submission True}, \texttt{safe True}.

\textbf{Rebuild from caches:}
\begin{verbatim}
...
\textbf{Notebook:} Run all cells in \texttt{Project\_Analysis.ipynb} (first run builds window feature cache).
